<div class="max-w-7xl mx-auto px-4 py-8">
    ...
    @if(!empty($topFailingChecks))
        {{-- NG Drilldown Analytics --}}
        <div class="mb-8">
            <h3 class="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
                <svg class="w-5 h-5 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>
                Top Failing Checks (NG Drilldown)
            </h3>
            <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
                @foreach($topFailingChecks as $task => $count)
                <div class="bg-red-50/50 border border-red-100 rounded-2xl p-4 flex items-center gap-4 shadow-sm hover:shadow transition-all group">
                    <div class="flex-shrink-0 bg-white rounded-xl w-12 h-12 flex items-center justify-center border border-red-100 shadow-sm group-hover:scale-105 transition-transform">
                        <span class="text-red-600 font-black text-xl">{{ $count }}</span>
                    </div>
                    <div>
                        <h4 class="text-sm font-bold text-slate-800 leading-tight mb-0.5 line-clamp-2" title="{{ $task }}">{{ $task }}</h4>
                        <p class="text-[10px] uppercase font-bold text-red-500 tracking-wider">Failures</p>
                    </div>
                </div>
                @endforeach
            </div>
